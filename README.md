...
在 Windows 上构建桌面版需要 Python 3.11、Node.js、Rust MSVC 工具链和 WebView2。
...
安装包生成在 `apps/desktop/src-tauri/target/release/bundle/nsis/`。首次构建会自动准备
Python sidecar 环境并构建前端。桌面数据保存在当前 Windows 用户的应用数据目录。

Web 开发步骤见 [`docs/current/LOCAL_DEVELOPMENT.md`](docs/current/LOCAL_DEVELOPMENT.md)。
演示流程和案例见 [`STRA 演示脚本`](docs/current/portfolio/STRA_DEMO_SCRIPT.md) 与
[`产品案例`](docs/current/portfolio/STRA_PORTFOLIO_CASE_STUDY.md)。完整文档索引见
