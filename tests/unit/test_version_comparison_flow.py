...
def test_version_comparison_reports_four_window_acquisition_progress(monkeypatch):
    event_a = _event("a" * 8, "2026-01-10")
    event_b = _event("b" * 8, "2026-03-10")
    request = route.VersionComparisonRequest(
        app_id=570,
    ...
    updates = []
    monkeypatch.setattr(route.acquisition, "ensure_reviews", lambda contract: _complete_result([{"recommendationid": "one"}]))
    monkeypatch.setattr(route, "_update_running", lambda run_id, phase, progress=None: updates.append((run_id, phase, progress)))

    route._acquire_population(request, event_a, event_b, run_id="run-progress")

    completed = [progress for _, _, progress in updates if progress.get("reviews_collected") is not None]
    assert [item["completed"] for item in completed] == [1, 2, 3, 4]
    assert all(item["total"] == 4 for item in completed)
    assert all(item["reviews_collected"] == 1 for item in completed)
