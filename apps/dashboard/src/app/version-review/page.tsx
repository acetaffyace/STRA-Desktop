...
function VersionReviewContent() {
  ...
  const copy = language === 'zh'
    ? { eyebrow: '版本复盘', intro: '选择游戏开始复盘，或打开已有记录。', placeholder: '粘贴已有运行 ID', load: '打开已有复盘', loading: '加载中…', empty: '选择游戏开始版本复盘。', priority: '重点问题', evidence: '证据卡片', actions: '建议行动', topics: '新兴话题', limits: '局限性' }
    : language === 'ja'
      ...
      <PageTransition>
        <div className="min-h-screen bg-[rgb(5,5,15)] px-4 py-8 text-slate-100 sm:px-8 lg:px-12">
          <div className="mx-auto max-w-7xl space-y-6">
            <div>
              <h1 className="text-3xl font-semibold tracking-tight text-white">版本复盘</h1>
            ...
            <Card variant="gradient">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
                <div className="flex-1">
                  <p className="text-xs uppercase tracking-wider text-cyan-300">创建版本复盘</p>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex-1">
                    <label htmlFor="event-select" className="text-xs uppercase tracking-wider text-slate-500">当前版本</label>
                  <select
                    id="event-select"
                ...
                {analysisMode === 'version_comparison' && (
                  <div className="flex-1">
                    <label htmlFor="comparison-event-select" className="text-xs uppercase tracking-wider text-slate-500">对比版本</label>
                    <select
                ...
                <div className="flex-1">
                  <label htmlFor="window-days" className="text-xs uppercase tracking-wider text-slate-500">时间窗口</label>
                  <select
                    id="window-days"
                    value={windowDays}
            ...
            {recentVersionReviews.filter((item) => !hiddenRunIds.has(item.run_id)).length > 0 && (
              <Card variant="glass" padding="sm">
                <div className="flex items-center justify-between gap-3">
                  <div><h2 className="text-lg font-semibold text-white">最近版本复盘</h2></div>
                </div>
            ...
            {run?.status === 'running' && !hasResultMetrics && (
              <Card variant="gradient">
                ...
                <p className="mt-2 text-xs text-slate-500">每 3 秒更新 · Run {run.run_id.slice(0, 8)}</p>
              </Card>
            )}
              ...
              <div className="space-y-6">
                ...
                <Card variant="gradient">
                  <div className="flex flex-wrap items-end justify-between gap-4">
                    <div><p className="text-xs uppercase tracking-[0.2em] text-cyan-300">{gameLabel}</p><h2 className="mt-2 text-2xl font-semibold text-white">{versionReviewDateA} / {versionReviewDateB}</h2></div>
            ...
            {hasResultMetrics && metrics && run && !hasV2 && (
              <>
                ...
                <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
                  <Card>
                    <div className="flex items-center justify-between gap-4">
                      <div><h2 className="text-lg font-semibold text-white">{copy.priority}</h2><p className="mt-1 text-xs text-slate-500">按评论提及次数统计。</p></div>
                  ...
                  <Card>
                    <div className="flex items-center justify-between gap-4"><h2 className="text-lg font-semibold text-white">{copy.evidence}</h2><Badge variant="success" size="sm">{(metrics.evidence_cards || []).length} 条证据</Badge></div>
                    <div className="mt-5 space-y-3">
                ...
                <Card>
                  <div className="flex items-center justify-between gap-4"><h2 className="text-lg font-semibold text-white">{copy.actions}</h2><Badge variant="warning" size="sm">待验证</Badge></div>
