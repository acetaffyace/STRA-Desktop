...
import { buildDashboardRunUrl, buildVersionReviewRunUrl, fetchActiveAnalysisRuns, fetchProgress, fetchRunLlmCost, type AnalysisHistoryItem, type RunLlmCost } from '@/lib/api';
...
function LlmActivity({ cost, provider, model }: { cost?: RunLlmCost; provider?: string | null; model?: string | null }) {
  if (!cost || cost.summary.call_count <= 0) return null;
  const completed = cost.calls.filter((call) => call.status === 'completed').length;
  const running = cost.calls.filter((call) => call.status === 'started').length;
  const failed = cost.calls.filter((call) => call.status === 'failed').length;
  const resolvedProvider = cost.calls[0]?.provider || provider;
  const resolvedModel = cost.calls[0]?.model_id || model;
  return (
    <div className="rounded-lg border border-violet-400/20 bg-violet-400/[0.06] px-2.5 py-2 text-[11px] text-violet-100">
      <p className="font-medium">LLM 实际调用 · {resolvedProvider || 'provider'}{resolvedModel ? ` / ${resolvedModel}` : ''}</p>
      <p className="mt-1 text-violet-200/80">已完成 {completed} / {cost.summary.call_count} 次{running > 0 ? ` · 进行中 ${running}` : ''}{failed > 0 ? ` · 失败 ${failed}` : ''}{cost.summary.input_tokens + cost.summary.output_tokens > 0 ? ` · ${(cost.summary.input_tokens + cost.summary.output_tokens).toLocaleString()} tokens` : ''}</p>
    </div>
  );
}
...
export function AnalysisWidget() {
  ...
  const [remoteProgress, setRemoteProgress] = useState<Record<string, Awaited<ReturnType<typeof fetchProgress>>>>({});
  const [llmCosts, setLlmCosts] = useState<Record<string, RunLlmCost>>({});
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const items = await fetchActiveAnalysisRuns();
        const [progressEntries, costEntries] = await Promise.all([
          Promise.all(items.filter((run) => run.run_type === 'general_analysis').map(async (run) => [run.run_id, await fetchProgress(run.app_id).catch(() => null)] as const)),
          Promise.all(items.map(async (run) => [run.run_id, await fetchRunLlmCost(run.run_id).catch(() => null)] as const)),
        ]);
        if (cancelled) return;
        setRemoteRuns(items);
        setRemoteProgress((previous) => ({ ...previous, ...Object.fromEntries(progressEntries.filter((entry): entry is readonly [string, Awaited<ReturnType<typeof fetchProgress>>] => entry[1] !== null)) }));
        setLlmCosts((previous) => ({ ...previous, ...Object.fromEntries(costEntries.filter((entry): entry is readonly [string, RunLlmCost] => entry[1] !== null)) }));
        setQueueError(null);
      } catch (error) {
        if (!cancelled) setQueueError(error instanceof Error ? error.message : '分析队列无法连接到当前后端。');
      }
    };
    refresh();
  ...
  const visibleRemoteRuns = remoteRuns.filter((run) => !activeTasks.some(([, task]) => task.progress?.run_id === run.run_id || task.result?.run_id === run.run_id || (task.game.appid === run.app_id && task.status === 'analyzing')));
  const hasActiveTasks = activeTasks.length > 0 || visibleRemoteRuns.length > 0;
        ...
        <div className="flex items-center justify-between border-b border-white/10 bg-slate-800/50 px-4 py-3">
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-semibold text-white">
              {ui.queue} ({activeTasks.length + visibleRemoteRuns.length})
            </h3>
          </div>
          <div className="flex items-center gap-2">
            <button
          ...
          <div className="max-h-96 space-y-2 overflow-y-auto p-4">
            ...
            {activeTasks.map(([appId, task]) => {
              const remainingLabel = task.progress
                ...
                : null;
              const runId = task.progress?.run_id || task.result?.run_id || undefined;
              const llmCost = runId ? llmCosts[runId] : undefined;
              return (
                <div
                key={appId}
                ...
                <div className="flex-1 space-y-2">
                  ...
                  {task.status === 'completed' && (
                    <div className="flex items-center gap-2 text-xs text-emerald-400">
                      ...
                      <Link
                        href={task.result?.run_id ? buildDashboardRunUrl(appId, task.result.run_id) : `/dashboard?game=${appId}`}
            ...
            {visibleRemoteRuns.map((run) => {
              ...
              const liveProgress = remoteProgress[run.run_id];
              const current = Number(liveProgress?.processed ?? contract.classified_count ?? run.classified_count ?? 0);
              const total = Number(liveProgress?.total ?? run.analysis_population_count ?? run.requested_review_count ?? 0);
              return (
                <div key={run.run_id} className="flex items-start gap-3 rounded-xl border border-cyan-400/20 bg-slate-800/30 p-3">
                  ...
                  <div className="flex-1 space-y-1.5"><div className="flex items-start justify-between gap-2"><p className="text-sm font-medium text-white line-clamp-1">{run.app_name}</p><span className="text-[10px] text-cyan-300">{isVersionReview ? '版本复盘' : '常规分析'}</span></div><p className="text-xs text-cyan-200">{phaseLabel}</p><p className="text-[11px] text-slate-400">{total > 0 ? `${current} / ${total} 条评论` : '正在准备分析方案…'}</p><div className="h-1.5 overflow-hidden rounded-full bg-slate-700"><div className="h-full bg-gradient-to-r from-cyan-500 to-indigo-500 transition-all" style={{ width: `${total > 0 ? Math.min(100, current / total * 100) : 25}%` }} /></div><LlmActivity cost={llmCosts[run.run_id]} provider={run.provider} model={run.model_id}/><Link href={isVersionReview ? buildVersionReviewRunUrl(run.run_id) : buildDashboardRunUrl(run.app_id, run.run_id)} className="text-[11px] text-sky-400 hover:underline">查看当前运行</Link></div>
                </div>
              );
            })}
          </div>
