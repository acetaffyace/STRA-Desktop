...
def resolve_version_event_anchor(
    event_id: str,
    *,
    effective_at: str,
    anchor_precision: str = "day",
    source_quality: str = "official_news_title_date",
) -> Optional[Dict[str, Any]]:
    """Promote an official discovered event to a usable dated anchor.

    Steam News synchronization is intentionally separate from review
    acquisition.  Once an official news item supplies a stable publication
    date, persist that date explicitly so version-comparison planning does not
    strand the event in the unresolved state.
    """
    from . import db as db_module
    ...
    return get_version_event(event_id)
