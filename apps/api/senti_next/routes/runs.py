...
def sync_news_version_events(app_id: int, request: NewsEventSyncRequest = NewsEventSyncRequest()) -> dict:
    ...
    existing_by_source = {str(item.get("source") or ""): item for item in existing if item.get("source")}
    existing_by_url = {str(item.get("source_url") or ""): item for item in existing if item.get("source_url")}
    seen_sources = set(existing_by_source)
    seen_urls = set(existing_by_url)
    created: List[dict] = []
    ...
    for news_item in news_items:
        ...
        if not candidate or not candidate["gid"]:
            continue
        duplicate = existing_by_source.get(candidate["source"])
        if duplicate is None and candidate["source_url"]:
            duplicate = existing_by_url.get(candidate["source_url"])
        if duplicate is not None:
            if (
                str(duplicate.get("event_status") or "").lower() in {"unresolved", "insufficient", "conflicted"}
                and candidate.get("event_date")
                and str(duplicate.get("source") or "").startswith("steam_news:")
            ):
                promoted = storage.resolve_version_event_anchor(
                    str(duplicate["event_id"]),
                    effective_at=str(candidate["event_date"]),
                )
                if promoted:
                    resolved_existing.append(_event_response(promoted))
            continue
        ...
        event = storage.create_version_event({
            ...
            "effective_at": candidate["event_date"],
            "anchor_precision": "day",
            "source_quality": "official_news_title_date",
    ...
    return {
        ...
        "events_resolved": len(resolved_existing),
        "events": created,
        "resolved_events": resolved_existing,
