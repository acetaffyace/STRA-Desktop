...
def _acquire_population(
    ...
    *,
    ...
    completed_cohorts: dict[str, dict[str, Any]] = {}
    for index, cohort in enumerate(COHORTS, start=1):
        ...
        completed_cohorts[cohort] = {
            "reviews_collected": len(result.reviews),
            "source": result.source,
            "collection_complete": bool(report.get("collection_complete")),
            "truncated_by_max_reviews": bool(report.get("truncated_by_max_reviews")),
        }
        if run_id:
            _update_running(
                run_id,
