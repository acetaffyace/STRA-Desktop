...
fn inject_boot_error(handle: &tauri::AppHandle, error: &str) {
    ...
    let js = format!("window.__SENTINEXT_BACKEND_BOOT_ERROR__ = '{}';", escaped);
    ...
    tauri::Builder::default()
                ...
                Ok((_rx, child)) => {
                    ...
                    tauri::async_runtime::spawn(async move {
                        if !wait_for_backend(port).await {
                            inject_boot_error(&handle_clone, "本地分析服务启动超时，请重试。");
                        }
                    });
                }
                Err(err) => {
        ...
        .on_window_event(|window, event| {
            if let tauri::WindowEvent::Destroyed = event {
                ...
                if let Some(state) =
                    app.try_state::<Mutex<Option<tauri_plugin_shell::process::CommandChild>>>()
                {
